...
def run_talk(message: str, source: str = "user", source_event_id: int | None = None, metadata: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    trace.add("observe", "done", "사용자 입력을 이벤트로 저장하고 상호작용 상태를 갱신했다.", {"event_id": user_event_id, "source": source})
    ...
    trace.add(
        ...
        "done",
        "언어 해석과 정책 판단을 완료했다.",
        {"intent": (decision.get("language_interpretation") or {}).get("intent"), "risk": decision.get("risk_level")},
    )
    trace.add(
        "retrieve",
        "done",
        "관련 기억과 스킬 후보를 조회했다.",
        {"memories": len(decision.get("relevant_memories") or []), "skills": len(decision.get("relevant_skills") or [])},
    )
    trace.add(
        "plan",
        "done",
        "선택 목표와 응답 결정을 decision schema로 구성했다.",
        {"goal_id": decision.get("selected_goal_id"), "user_goal_created": decision.get("user_goal_created")},
    )
    if decision.get("user_goal_created"):
        trace.add(
            "act_or_defer",
            "deferred",
            "사용자 지시는 user queue 또는 프로젝트 계획으로 분리했다.",
            {"task_id": (decision.get("user_directed_goal") or {}).get("task_id")},
        )
    else:
        trace.add("act_or_defer", "deferred", "대화 응답만 필요한 입력이라 실행 작업은 만들지 않았다.", {})
    ...
    log_event(
        "core_pipeline",
        "pipeline_trace_completed",
        json.dumps(decision["pipeline_trace"], ensure_ascii=False),
        {"decision_id": decision_row_id, "goal_id": decision.get("selected_goal_id")},
        0.55,
    )
    return {
        "text": output,
