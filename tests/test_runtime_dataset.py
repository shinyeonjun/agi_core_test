...
def test_runtime_replay_includes_accumulated_interaction_contract(tmp_path):
    db = tmp_path / "harness.db"
    service = HarnessService(db_path=db, project_root=tmp_path)
    task = {
        "goal": "메모리 확인",
    ...
    assert manifest["extraction"]["interaction_outcomes_seen"] == 1
    assert rows[0]["interaction"]["interaction_aligned"] is True
    assert rows[0]["interaction"]["required_outputs"] == ["memory_usage"]
    assert rows[0]["interaction"]["answered_outputs"] == ["memory_usage"]
    assert rows[0]["interaction"]["missing_outputs"] == []
    assert rows[0]["interaction"]["answer_quality"] == "complete"
