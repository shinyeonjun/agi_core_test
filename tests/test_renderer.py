...
def test_fallback_renderer_contains_contract_tokens():
    decision = {"version": "0.15", "selected_goal": {"id": 1, "title": "Answer user input"}, "drive_scores": {"completion": 0.2}, "policy_summary": {"risk_level": "low", "requires_approval": False}, "relevant_memories": [], "relevant_skills": [], "renderer": "fallback"}
...
def test_codex_validator_rejects_unsafe_command():
    result = validate_codex_output("v0.6 Core event goal sudo apt install nginx", {"must_include": ["v0.15", "Core", "event", "goal"], "must_not_include": []})
    assert result["ok"] is False
