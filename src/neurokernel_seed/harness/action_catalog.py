...
def default_action_catalog() -> dict[str, ActionDefinition]:
    actions = [
        ActionDefinition("get_uptime", "시스템 업타임 조회", "low", False, False, "readonly_system", description="시스템 업타임을 read-only로 조회한다.", examples=("오렌지파이 켜진 지 얼마나 됐어?", "업타임 알려줘")),
        ActionDefinition("get_disk_usage", "디스크 사용량 조회", "low", False, False, "readonly_system", {"path": {"type": "string", "default": "."}}, description="디스크 사용량을 read-only로 조회한다.", examples=("디스크 용량 확인해줘", "남은 저장공간 알려줘")),
        ActionDefinition("get_memory_usage", "메모리 사용량 조회", "low", False, False, "readonly_system", description="메모리 사용량을 read-only로 조회한다.", examples=("메모리 상태 어때?", "램 사용량 알려줘")),
        ActionDefinition("get_cpu_temp", "CPU 온도 조회", "low", False, False, "readonly_system", description="CPU 온도를 read-only로 조회한다.", examples=("CPU 온도 알려줘", "오렌지파이 뜨거워?")),
        ActionDefinition("get_cpu_per_core_usage", "CPU 코어별 사용률 조회", "low", False, False, "readonly_system", description="CPU 코어별 사용률을 read-only로 조회한다.", examples=("CPU 코어별 사용률 보여줘", "각 코어 얼마나 쓰고 있어?")),
        ActionDefinition("get_service_status", "서비스 상태 조회", "low", False, False, "readonly_system", {"service": {"type": "string"}}, description="지정한 systemd 서비스 상태를 read-only로 조회한다."),
        ActionDefinition("tail_logs", "로그 꼬리 조회", "low", False, False, "readonly_system", {"path": {"type": "string"}, "lines": {"type": "integer", "default": 80}}, description="허용된 프로젝트 경로의 로그 마지막 줄을 조회한다."),
        ActionDefinition(
            "list_artifacts",
            "모델 파일 위치와 artifact 목록 조회",
            "low",
            False,
            False,
            "readonly_system",
            {"path": {"type": "string", "default": "artifacts"}},
            description="artifact 디렉터리의 모델 파일 위치, 파일 이름, ONNX, RKNN, pt, manifest 산출물 목록을 read-only로 조회한다.",
            examples=("모델 파일 어디 있어?", "모델파일 이름 뭐야?", "ONNX/RKNN 파일 목록 보여줘"),
        ),
        ...
        ActionDefinition(
            ...
            description="CPU, 메모리, 디스크, 온도, 로그, 최근 trace, artifact 상태를 묶어 증상 기반 진단 요약을 만든다.",
            examples=("느려짐 원인 진단해줘", "학습 멈춘 것 같아", "모델 파일 이상한지 진단해줘"),
        ),
        ActionDefinition(
        ...
        ActionDefinition(
            "inspect_model_usage",
            "World/runtime model usage inspection",
            "low",
            False,
            False,
            "readonly_system",
            description="Read-only audit of world/runtime model slots, planner connections, runtime ranking data, and unsafe live-rollout gaps.",
            examples=("world/runtime model usage audit", "inspect model planner connections"),
        ),
