...
class RknnWorldModelPredictor(BasePredictor):
    name = "learned-rknn"
    ...
    def __init__(self, model_path: str | Path):
        try:
            ...
            raise RuntimeError("rknn-toolkit-lite2 is required to use RknnWorldModelPredictor on Orange Pi") from exc
        ...
        if ret != 0:
            raise RuntimeError(f"RKNN load failed with code {ret}: {self.model_path}")
        ret = self.runtime.init_runtime(core_mask=RKNNLite.NPU_CORE_AUTO)
        if ret != 0:
            raise RuntimeError(f"RKNN runtime init failed with code {ret}")

    def predict(self, state: WorldState, action: Action) -> Prediction:
        import numpy as np

        input_vector = self._encode_input(state, action)
        outputs = self.runtime.inference(inputs=[np.array([input_vector], dtype=np.float32)])
        output = outputs[0][0]
