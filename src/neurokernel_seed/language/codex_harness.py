...
from .prompt_pipelines import human_current_pipeline_prompt
...
def _scope_of_agency_prompt(*, mode: str) -> str:
    return human_current_pipeline_prompt(mode=mode)
...
def _to_human_prompt(core_result: dict[str, Any], *, style: str, context: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            "실패, 차단, 승인대기는 숨기지 않는다.",
            "System Result.kind가 work_status이면 progress와 lifecycle을 우선 읽고, 사용자가 알아야 할 현재 단계, 다음 주체, 사용자 승인 필요 여부를 말한다.",
            "work_status에서 planned가 recorded_plan_without_active_worker이면 자동 구현 중이라고 말하지 않는다. 계획으로 접수됐고 다음 실행 워커/작업 전개가 필요하다고 말한다.",
            "work_status에서 waiting_approval이면 장착 또는 결과 승인 버튼을 눌러야 다음 단계로 간다고 말한다.",
            "work_status에서 reviewing, blocked, failed이면 수정/재시도 또는 리뷰가 필요하다고 말한다.",
...
def _work_route_prompt(user_text: str, context: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            "- For work_status, set work_item=null. Do not create a new work item just because the user asks what is currently pending.",
            ...
            'User: "현재 작업 상태 보여줘" -> route=work_status, work_item=null',
            'User: "큐에 남은 개발 작업 뭐 있어?" -> route=work_status, work_item=null',
